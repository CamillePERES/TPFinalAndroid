 ...
 *
 * ici on a un singleton, car on va avoir 1 instance de la base
 */
@Database(
...
//RoomDatabase : all classes that are annotated with Database must extend this class.
        ...
        private fun insertFakeData() {
            Executors.newSingleThreadExecutor().execute {
                InMemory_NeighborS.forEach {
                    instance?.neighborDao()?.add(it.toNeighborEntity())
                }
            }
        }
